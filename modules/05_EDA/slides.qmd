...
## {.iclicker}
:::{.question}
How much time do you spend outside of class on AREC 330 each week?
:::
...
B. Mixed (some easy, some hard)   
C. Very difficult  
:::

## {.iclicker}
:::{.question}
How can I improve the class? (constructive feedback)
:::
...
flowchart LR
  ...
  style EDA fill:#FFD966,stroke:#333,stroke-width:3px,color:#000
```



## Before You Model,<br> You Must Understand
...
- Are there clusters?
- Are there outliers driving some relationship?
...
:::{.notes}
What if we were interested in how drought affects individual farms instead of state-level GDP? Would our data be suitable for answering that question?
:::
...
- Is the mean representative of the average US state Farm GDP?
- Would you consider transforming the variable? Why?
- How might this affect an analysis of the relationship between drought and farm GDP?
...
Example: For {1, 2, 3, 4, 5}, variance = 2.5
...
Example: For {1, 2, 3, 4, 5}, standard deviation = $\sqrt{2.5} \approx 1.58$
...
- You may see interquartile range (IQR) = Q3 - Q1

Example: For {1, 2, 3, 4, 5}, 

- 2 is the 30th percentile, 
- 3 is the 50th percentile (median), 
- 4 is the 70th percentile.

Using the midpoint method for percentiles,
$$
P = \frac{R-0.5}{n} * 100
$$
where $R$ is the rank of the value in the sorted data and $n$ is the total number of observations.
